    ...
    <main class="min-w-0 min-h-screen flex flex-col lg:h-screen lg:flex-1 lg:overflow-hidden">
        <div
            ...
            <div class="flex min-w-0 items-start gap-3">
                ...
                <div class="min-w-0">
                    <h2 class="text-lg font-extrabold sm:text-xl">Good morning, {{ user.name }} 👋</h2>
                    <div class="text-[13px] text-slate-500 mt-0.5">{{ momemt.format('dddd') }}, {{momemt.format('m')}} {{ momemt.date() }} · 2 reminders due today</div>
                </div>
            </div>
            <div class="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:justify-end sm:gap-2.5">
                <button
                ...
                <img :src="user.image"  class="hidden w-[34px] h-[34px] rounded-full cursor-pointer sm:flex"/>
            </div>
        </div>
    </main>
</template>
